...
export default function ProjectCard({ project, isEdit = false }: { project: Project, isEdit: boolean }) {
  return (
      ...
      >
        ...
        <CardFooter className='before:bg-white/10 border-white/20 border-1 overflow-hidden py-1 absolute before:rounded-xl rounded-large bottom-1 w-[calc(100%_-_8px)] shadow-small ml-1 z-10'>
          <div className='flex flex-col gap-y-1'>
            ...
            <p className='line-clamp-2 md:line-clamp-3 text-white'>{project.description[0]}</p>
          </div>
        </CardFooter>
      </Card>
    </Link>
